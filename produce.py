...
parser.add_argument('--static-comment', type=str, default='ospf', 
                    help='Static comment for ip route add command')
parser.add_argument('--static-gateway', type=str, default='192.168.88.253', 
                    help='Static gateway for ip route add command')
...
def dump_iproute(lst, f, static_comment, static_gateway):
    for n in lst:
        if n.dead:
            continue
        if len(n.child) > 0:
            dump_iproute(n.child, f, static_comment, static_gateway)
        elif not n.dead:
            # 构造路由字符串
            route_str = 'ip route add dst-address={} comment={} gateway={};'.format(n.cidr, static_comment, static_gateway)
            # 写入路由配置到文件
            f.write(route_str + '\n')
...
with open("routes4.rsc", "w") as f:
    dump_iproute(root, f, args.static_comment, args.static_gateway)
